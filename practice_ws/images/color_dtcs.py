...
def d_coke(img):
    ...
    # BGR空間での抽出範囲
    ## コーラ缶
    lower = np.array([40, 50, 50])   # 色相, 彩度, 明度 の下限
    upper = np.array([80, 255, 255]) # 色相, 彩度, 明度 の上限
...
def d_redcoke(img):
    ...
    # BGR空間での抽出範囲
    ## コーラ缶
    lower = np.array([170, 100, 100]) # 色相の下限（170〜179）
    upper = np.array([ 10, 255, 255]) # 色相の上限（0〜10）
